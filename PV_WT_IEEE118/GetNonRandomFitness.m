...
% * 从IEEE118_non_random_Result中获取适应度值
...
for i = 1:length(Test_name)
    for j = 1:length(Method_name)
        Fitness(i, j) = IEEE118_non_random_multi_Result.(Test_name_fields(i)).(Method_name_fields(j)).Best_fitness;
        Mean_Fitness(i, j) = IEEE118_non_random_multi_Result.(Test_name_fields(i)).(Method_name_fields(j)).Mean;
        Std_Fitness(i, j) = IEEE118_non_random_multi_Result.(Test_name_fields(i)).(Method_name_fields(j)).Std;
        Worst_Fitness(i, j) = IEEE118_non_random_multi_Result.(Test_name_fields(i)).(Method_name_fields(j)).Worst_fitness;
    end
end

% * 存入excel文件，文件名为IEEE118_non_random_Fitness.xlsx
File_name = "IEEE118_non_random_Fitness.xlsx";
% * 写入最优值，平均值，最差值，方差
% * 第一行是算法名，从 B1 开始
