...
function [best_Gene,bestGene_Position,bestfitGene] = GA(ObjFun,LB,UB,nVar,nPop,MaxIt)
    %% Problem Parameters
    ...
    bestfitGene = NaN(Maxiter,1);  % vetcor to store best fitness fun value per iteration
    ...
    for t = 1:Maxiter
        ...
        bestfitGene(t) = min(f);
        %disp(['Iteration ' num2str(t) ': Best Cost = ' num2str(bestfitGene(t))]);
    end
    % Results
    [best_Gene,ind] = min(f);
    bestGene_Position = P(ind,:);
    disp(['GA: Best Cost = ' num2str(best_Gene)]);
    end
