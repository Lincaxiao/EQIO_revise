function [Fgb,Objx,gb,PositionValue]=CDEO2(UB,LB,PVN,MH)
function [Best_score, Best_pos, curve] = WSTO(fobj, lb, ub, dim, N, Max_iter)
% global MPN MIT
MPN = N;        % 质点数量 (Population size)
MIT = Max_iter; % 最大迭代次数 (Maximum iterations)
PVN = dim;      % 维度 (Dimensionality)
MH = fobj;      % 目标函数句柄 (Objective function handle)
LB = lb;        % 变量下界 (Lower bounds)
UB = ub;        % 变量上界 (Upper bounds)
