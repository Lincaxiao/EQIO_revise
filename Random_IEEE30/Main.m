...
if exist('IEEE30_random_Result.mat', 'file')
    load('IEEE30_random_Result.mat');
    Result = IEEE30_random_Result;
end
% 基础的费用测试同样包括了风电的参与
...
for i = 1:length(Test_name)
    ...
    for Algorithm_Index = 1:8
        ...
        %% 创建一个数组以容纳100张IEEE30数据的结果
        IEEE30_Result = zeros(100, 25); % 100张数据，每张数据25个参数
        IEEE30_Fitness = zeros(100, 1);
        parfor Grid_Index = 1:100
            ...
            %% 现在best_p中存储的是最优解
            IEEE30_Result(Grid_Index, :) = best_position;
            [a, b, c, func] = Get_function_without_penalty(Test_name(i), Grid_Index);
            %% 储存对应的适应度值
            IEEE30_Fitness(Grid_Index) = func(IEEE30_Result(Grid_Index, :));
            All_Fitness(Grid_Index, Algorithm_Index) = IEEE30_Fitness(Grid_Index);
        end
        %% 现在将适应度值储存在结构体中
        Tmp_array = struct('best_particle', IEEE30_Result, 'best_fit', IEEE30_Fitness);
...
IEEE30_random_Result = Result;
save('IEEE30_random_Result.mat', 'IEEE30_random_Result');
