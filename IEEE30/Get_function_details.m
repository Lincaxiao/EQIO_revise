function [lb, ub, dim, fobj] = Get_function_details(function_name)
    dim = 25;
    ...
    switch function_name
        case {'30_Cost_Base', '30_Cost_with_Tax', '30_Ploss', '30_Voltage_Deviation', ...
                '30_Cost_Ploss', '30_Cost_VD', '30_Cost_Tax_Ploss_VD', '30_L_index'}
            fobj = @(x) Fun_ieee30_without_VPLE(x, function_name);
        case '30_Cost_with_VPLE'
            fobj = @(x) Fun_ieee30_with_VPLE(x);
    end
end

function y = Fun_ieee30_without_VPLE(x, function_name)
    ...
    for i = 1:30
        if V(i) > Vmax
            penalty = penalty + (V(i) - Vmax) * 10000 + 50;
        end
        if V(i) < Vmin
            penalty = penalty + (Vmin - V(i)) * 10000 + 50;
        end
    end
    VD = sum(abs(V - 1));
    ...
    switch function_name
        ...
        case '30_Cost_with_Tax'
            y = Cost + Cal_Tax(x) + penalty;
        case '30_Ploss'
            y = Ploss + penalty;
        case '30_Voltage_Deviation'
            y = VD + penalty;
        case '30_Cost_Ploss'
            y = Cost + 40 * Ploss + penalty;
        case '30_Cost_VD'
            y = Cost + 100 * VD + penalty;
        case '30_Cost_Tax_Ploss_VD'
            y = Cost + Cal_Tax(x) + 21 * Ploss + 22 * VD + penalty;
        case '30_L_index'
            y = calculate_L_index_max(res) + penalty;
    end
    if res.success == 0
        y = 1e10;
    end
...
function y = Fun_ieee30_with_VPLE(x)
    %% 30_Cost_with_VPLE
    %       V1,   V2,   V5,   V8,   V11,  V13   T11   T12   T15   T36  
    %       Q10   Q12   Q15   Q17   Q20   Q21   Q23   Q24   Q29
    xMax = [1.10, 1.10, 1.10, 1.10, 1.10, 1.10, 1.10, 1.10, 1.10, 1.10...
    ...
    for i = 1:6
        P = res.gen(i, 2);
        Cost = Cost + res.gencost(i, 5) * P^2 + res.gencost(i, 6) * P + Valve_Point_Coefficient(i, 1) * abs(sin(P * Valve_Point_Coefficient(i, 2)));
    end
    Cost = Cost + Cal_PV_WT_Cost(x(22), 1, 1) + Cal_PV_WT_Cost(x(24), 1, 2) + Cal_PV_WT_Cost(x(25), 2, 1);
    penalty = 0;
    for i = 1:25
